...
MolPolMessenger::~MolPolMessenger(){
  ...
  if(fMolPolDir)   delete fMolPolDir;
  if(seedCmd)      delete seedCmd;
  if(genSelectCmd) delete genSelectCmd;
  if(fXminCmd)     delete fXminCmd;
  if(fXmaxCmd)     delete fXmaxCmd;
  if(fYminCmd)     delete fYminCmd;
  if(fYmaxCmd)     delete fYmaxCmd;
  if(fBeamECmd)    delete fBeamECmd;
  if(fEminCmd)     delete fEminCmd;
  if(fEmaxCmd)     delete fEmaxCmd;
  if(fthetaMinCmd) delete fthetaMinCmd;
  if(fthetaMaxCmd) delete fthetaMaxCmd;
  if(fphiMinCmd)   delete fphiMinCmd;
  if(fphiMaxCmd)   delete fphiMaxCmd;
  if(fZCmd)        delete fZCmd;
  if(fthetaComMinCmd) delete fthetaComMinCmd;
  if(fthetaComMaxCmd) delete fthetaComMaxCmd;
